...
class SqlEnv(IntercodeEnv):
    ...
    def get_reward(self) -> Tuple[float, Dict]:
        ...
        # Calculate Rewards
        if isinstance(self.info[AGENT_OBS], list) and isinstance(
            self.info[EVAL_OBS], list
        ):
            ...
            merged_data = sorted(
                chain(dist_agent.items(), dist_eval.items()), key=get_key
            )
            union = {
                k: max(map(get_val, g)) for k, g in groupby(merged_data, key=get_key)
            }
            ...
            if len(union) == 0:
                ...
                self.info[REWARD] = total_intersect * 1.0 / total_union

                # Reward: Scale reward by sorting accuracy
                if len(intersection) > 0:
                    list_agent_intx = self.get_intersect_items(
                        list_agent.copy(), intersection.copy()
                    )
                    list_eval_intx = self.get_intersect_items(
                        list_eval.copy(), intersection.copy()
                    )
                    ...
                    if not math.isnan(sort_correlation) and isinstance(
                        sort_correlation, float
                    ):
                        self.info[REWARD] = round(
                            sort_correlation * self.info[REWARD], 2
                        )
        else:
