...
from experiments.policies import PalmChatPolicy, PalmCompletionPolicy, HFChatPolicy
...
parser.add_argument(
    "--policy", choices=["chat", "complete"], help="policy to use for evaluation"
)
parser.add_argument(
    "--template",
    choices=["v1", "v2", "v3", "game", "game_sql", "v4"],
...
class ExperimentWrapper:
    def __init__(self, args):
        ...
        log_file_name = (
            f"{self.env.name}_multiturn_starchat_{args.max_turns}_turns.json"
        )
        ...
        if args.policy == "chat":
            ...
            self.policy = PalmCompletionPolicy(
                language=args.env,
                setting=SETTING_MAP[args.env],
                template=args.template,
                dialogue_limit=args.dialogue_limit,
                ...
                for turn in range(self.args.max_turns):
                    ...
                    if self.args.verbose:
                        ...
                        if isinstance(observation, str) and observation.startswith(
                            f"No {self.policy.language} code"
                        ):
                            print(
