...
def bash_parser(action: str):
    ...
    pattern2 = f"```([\S\s]+?)```"
    matches = re.findall(pattern1, action.lower(), re.DOTALL) + re.findall(
        pattern2, action.lower(), re.DOTALL
    )
    if len(matches) == 0:
        return action, True
...
def gen_react_demos(file_name, num_demos) -> list:
    ...
    for _, v in trajectories.items():
        if v["summary"]["max_reward"] == 1.0 and len(v["turn_history"]["actions"]) >= 3:
            good_demos.append({"query": v["query"], "sequence": v["turn_history"]})
    ...
    for demo in good_demos:
        ...
        for turn in range(1, len(demo["sequence"]["actions"]) + 1):
            ...
            react_demo_str += (
                f"Action {turn}: execute[{demo['sequence']['actions'][turn - 1]}]\n"
            )
