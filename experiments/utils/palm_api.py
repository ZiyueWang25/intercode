...
@tenacity.retry(
    wait=tenacity.wait_random_exponential(min=150, max=500),
...
class PalmChat:
    ...
    def init_chat(self, context="You are a helpful assisstant", init_message="Hello"):
        # initialize chat
        response = None
        response = retry_chat(**self.defaults, context=context, messages=init_message)
...
def PalmCompletion(phrase, model="models/text-bison-001", num_samples=1):
    ...
    sleep(2)
    response = generate_text(**config, prompt=phrase)
...
if __name__ == "__main__":
    chat = PalmChat()
    chat.init_chat(
        init_message="I have 3 apples and 10 bananas. How many fruits do I have in total?"
    )
    for i in range(5):
