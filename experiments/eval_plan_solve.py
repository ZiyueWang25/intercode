...
from intercode.envs import BashEnv, SqlEnv, ACTION_EXEC
...
parser = argparse.ArgumentParser(
    description="Plan & Solve evaluation for Intercode environment"
)
parser.add_argument("--data_path", type=str, help="path to dataset to evaluate on")
parser.add_argument(
    "--env", choices=["sql", "bash"], help="Intercode environment to run eval on"
)
parser.add_argument(
    "--image_name", type=str, help="name of docker image to build environment with"
)
parser.add_argument(
    "--log_dir", type=str, help="folder to save experiment run log file to"
)
parser.add_argument(
    "--proportion", type=float, help="proportion of the dataset to use for evaluation"
)
parser.add_argument("--refine", action="store_true", help="whether to run refine step")
parser.add_argument(
    "--refine_turns", type=int, help="number of turns to run refine step for"
)
...
SETTING_MAP = {"sql": "MySQL Database", "bash": "Bourne Shell"}
...
class ExperimentWrapper:
    def __init__(self, args):
        ...
        if args.env == "sql":
            ...
            self.env = BashEnv(image_name=args.image_name, data_path=args.data_path)
        else:
        ...
        if args.refine and args.refine_turns:
            log_file_name = (
                f"{self.env.name}_plan_solve_refine_{args.refine_turns}_turns.json"
            )
        ...
        self.dialogue = []
        self.template = TemplatePlanSolve(
            self.args.env.upper(), SETTING_MAP[self.args.env]
        )
    ...
    def run_expr(self):
        try:
            ...
            if self.args.seed and self.args.proportion:
                indices = random.Random(self.args.seed).choices(
                    list(indices), k=int(len(indices) * self.args.proportion)
                )[6:]
            for idx in tqdm(indices, disable=self.args.verbose):
                ...
                self.dialogue.append(
                    {
                        "role": "user",
                        "content": self.template.get_query_msg(self.env.query),
                    }
                )
                action = llm(self.dialogue)
                plan = re.findall(
                    "\d+\.\s(.*?)(?=\s\d+\.|\Z)", action.content, re.DOTALL
                )
                ...
                self.dialogue.append(
                    {"role": "system", "content": self.template.get_execute_plan_msg()}
                )
                ...
                for idx_plan in range(len(plan)):
                    ...
                    turn_history["observations"].append(
                        str(observation)
                ...
                if reward != 1 and self.args.refine and self.args.refine_turns:
                    ...
                    self.dialogue.append(
                        {
                            "role": "system",
                            "content": self.template.get_after_plan_msg(),
                        }
                    )
            ...
            with open(self.log_path, "w") as fp:
                json.dump(
                    {"meta": vars(self.args), "logs": self.log_data}, fp, indent=2
                )
            self.env.close()
