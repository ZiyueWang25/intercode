...
parser.add_argument(
    "--data_path",
    type=str,
    default="./data/ctf/ic_ctf.json",
    help="path to bash queries json file",
)
parser.add_argument(
    "--max_turns", type=int, default=15, help="max number of interaction turns"
)
parser.add_argument(
    "--dialogue_limit",
    type=int,
...
parser.add_argument(
    "--log_dir", type=str, default=os.getcwd(), help="path to log directory"
)
    ...
    try:
        ...
        for idx in tqdm(range(len(data))):
            ...
            for turn in range(args.max_turns):
                action, is_code = policy.forward(
                    env.query, obs, reward, env.get_available_actions()
                )
            ...
            num_submissions = sum(
                1 for i in turn_history["rewards"] if i == 1 or i == -1
            )
